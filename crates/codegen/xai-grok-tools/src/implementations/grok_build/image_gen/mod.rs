//! `image_gen` tool — generates images via the Meta image API and saves
...
/// Default image model for `image_gen`. Used unless an explicit
/// `model_override` is supplied via `ImageGenConfig::Enabled`.
const META_IMAGE_MODEL: &str = "emu-image-quality";
// Some image models (e.g. `emu-image`, selectable via `model_override`)
// expand the prompt then generate, and the server buffers
...
/// HTTP client for the Meta image API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    http: reqwest::Client,
    base_url: String,
    /// Image model slug used by `generate()`. Selected at construction
    /// from `ImageGenConfig::model_override` (falling back to
    /// [`META_IMAGE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    edit_model: String,
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| META_IMAGE_MODEL.to_owned());
        let edit_model = edit_model_override
            .clone()
            .filter(|m| !m.trim().is_empty())
            .unwrap_or_else(|| super::image_edit::META_IMAGE_EDIT_MODEL.to_owned());
    ...
    pub async fn generate(
        ...
        if let Some(ref key) = self.current_bearer().await {
        ...
        if !status.is_success() {
            ...
            tracing::warn!(http_status = %status, "Image API error: {truncated}");
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// Optional image model override for `image_gen`. When `Some(non-empty)`,
        /// `image_gen` calls that model instead of the default quality model
        /// ([`META_IMAGE_MODEL`]). Driven by the `image_gen_model_override`
        /// config flag. `image_edit` is unaffected.
...
impl ImageGenConfig {
    ...
    /// default quality model ([`META_IMAGE_MODEL`]) is used.
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("emu-image".into()),
            edit_model_override: None,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("emu-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("emu-image")), None)
                .unwrap()
                .model,
            "emu-image"
        );
    ...
    fn client_selects_edit_model_from_override() {
        ...
        assert_eq!(
            ...
            super::super::image_edit::META_IMAGE_EDIT_MODEL
        );
        assert_eq!(
        ...
        let client = ImageGenClient::new(&mk(Some("emu-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "emu-image-v2");
        assert_eq!(client.model, META_IMAGE_MODEL);
    }
