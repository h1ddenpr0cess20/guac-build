...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade your plan for higher limits: https://example.invalid/upgrade";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("example.invalid/upgrade"));
    assert!(format_acp_error(&rpm, true).contains("example.invalid/upgrade"));
...
fn format_session_info_api_key_without_env() {
    ...
    assert!(
            text.contains("Run `guac login` to use a signed-in session instead."),
            "{text}"
        );
    assert!(!text.contains("console.x.ai"), "{text}");
    assert!(!text.contains("grok.com"), "{text}");
...
#[test]
fn format_session_info_session_auth_has_no_billing_line() {
    let info = make_session_info("auto", None, 1000, 10000);
